...
from plg.utils.androidutil import checkpackages
...
class Player(threading.Thread):
    ...
    def __terminate(self):
        logging.info("Terminate gets called")
        #XIANG
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                self.__run_explore(self.__timeout+10)
                ...
                logging.info('Giving up app %s Restart emulator' % self.__app_sha1)
                #self.__run_uninstall() 
                self.__launch_emu = True
                time.sleep(5)
    ...
    def __wait(self):
        ...
        logging.debug("    work on app:"+self.__app_name)
    ...
    def __run_install(self):
        try:
            ...
            install_thread.join(30)
            if install_thread.is_alive():
                logging.warning('    install process failed to terminate, force to terminate')
                sys.stdout.flush()
                if self.__install_process != None:
                    self.__install_process.terminate() 
                    install_thread.join(10)
                    raise PlayerException()
            logging.warning('    install thread done successfully')
        except Exception as e:
            logging.warning('    failed intalling app %s[%s] %s in_run_install'%(self.__app_name,self.__app_sha1, str(e)))
            raise PlayerException()
    ...
    def __install(self):
        ...
        try:
            ...
            logging.debug("    install cmd:"+" ".join(command))
            self.__install_process = subprocess.Popen(command)
            count = 0
            while self.__install_process.poll() is None:
                time.sleep(5)
                count += 1
                if count > 10:
                    logging.error("    install gets stuke2 terminat")
                    self.__install_process.terminate()
                    raise PlayerException()
            #subprocess.check_call(command, stderr=subprocess.STDOUT,timeout=30)
            #self.__install_process.start()
            plg.utils.logcat.clearlogcat(self.device)
            self.__logcat_proc = plg.utils.logcat.logcat(os.path.join(self.directory, 'logcat.txt'), self.device,self.__timeout)
            logging.info("        logcat pid "+str(self.__logcat_proc.pid)+" timeout:"+str(self.__timeout))
            sys.stdout.flush()
            ...
            logging.warning('    failed intalling app %s[%s] %s'%(self.__app_name,self.__app_sha1, str(e)))
            raise PlayerException()
    ...
    def __uninstall(self):
        try:
            ...
            count = 0
            while self.__uninstall_process.poll() is None:
                time.sleep(5)
                count += 1
                if count > 10:
                    logging.error("    uninstall process fails, needs to terminate")
        ...
        try:
            ...
            if uninstall_thread.is_alive():
                ...
                if self.__uninstall_process != None:
                    self.__uninstall_process.terminate() 
                    logging.warning('       done terminating process, start thread join')
                    sys.stdout.flush()
                    uninstall_thread.join(10)
                    logging.warning('       done thred join')
                    sys.stdout.flush()
                    raise PlayerException()
            logging.warning('    uninstall thread done successfully')
        except Exception as e:
            logging.warning('    2failed unintalling app %s[%s] %s in_run_uninstall'%(self.__app_name,self.__app_sha1, str(e)))
            raise PlayerException()
            
    def __run_explore(self,timeout):
        try:
            self.__explore_process = None
            explore_thread = threading.Thread(target=self.__explore)    
            explore_thread.start()

            explore_thread.join(timeout)
            if explore_thread.is_alive():
                logging.warning('    explore timeout force it to terminate')
                sys.stdout.flush()
                if self.__explore_process != None:
                    ...
                    raise PlayerException()
            self.__terminate()
            logging.warning('    explore thread done successfully')
            sys.stdout.flush()
        except Exception as e:
    ...
    #XIANG
    def __explore__(self,interval=300):
        try:
    ...
    def __explore(self):
        try:
            ...
            #plg.utils.androidutil.waitfordevice(self.device, timeout=60)
            logging.info("    start monkey")
            pkt_name = getapkname(self.__app_path)
            logging.info("    pkt's name "+pkt_name+" "+metainfo['name'])
            t = threading.Thread(target=run_monkey, args=(self.device, metainfo['name'],metainfo))
            t.daemon = True
            t.start()
            time.sleep(5)
            ...
            while True:
                ...
                if rs == None:
                    count += 1
                    pks = checkpackages(self.device)
                    #logging.debug("   show package:")
                    #for line in pks:
                    #    logging.error("    package:"+line)
                    #    sys.stdout.flush()
                    logging.warning("   alert!!! no monkey!")
                    time.sleep(5)
                    if count > 3:
                        logging.error("    fail launch monkey exit!")
                        raise PlayerException()
                else:
                    logging.info("    monkey is running "+' '.join(rs))
                    break
            
            logging.info('    start exploring: %s' % self.__app_path)
            sys.stdout.flush()
            self.__explore_process = Process(target=plg.explore.explore, args=(self.device,metainfo))
            self.__explore_process.start()
            self.__explore_process.join(self.__timeout)
            #metainfo['canstop'] = False
            logging.info('    done exploring: %s' % self.__app_path)
            sys.stdout.flush()
