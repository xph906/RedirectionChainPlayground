...
class Player(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                logging.info("wait for 5s and install next app")
                time.sleep(5)
                #self.__explore()
    ...
    def __launch(self):
        try:
            ...
            time.sleep(2)
            plg.utils.androidutil.waitfordevice(self.device, timeout=10)
    ...
    def __explore(self):
        try:
            timer = threading.Timer(600, self.__terminate)
            timer.start()
            metainfo = plg.metadata.getmetadata(self.__app_path)
            t = threading.Thread(target=run_monkey, args=(self.device, metainfo['name']))
            t.daemon = True
            t.start()
            logging.info('Exploring started: %s' % self.__app_sha1)
            plg.explore.explore(self.device, metainfo)
            logging.info('Exploring ... 1: %s' % self.__app_sha1)
            client = extra.notdet.Client(self.device, self.__port + 2)
            logging.info('Exploring ... 2: %s' % self.__app_sha1)
            client.init()
            logging.info('Exploring ... 3: %s' % self.__app_sha1)
            client.send()
            logging.info('Exploring ... 4: %s' % self.__app_sha1)
            timer.cancel()
            time.sleep(10)
